...
int MATRIXClient::insertNoVoHT(TaskDAG &dag, int &num_tasks, int &numSleep, ZHTClient &clientRet) {
...
int MATRIXClient::insertWaitQ(TaskDAG &dag, int &numSleep, int &num_tasks, int &computeSlots, int &max_tasks_per_package, ZHTClient &clientRet) {
	// Task description
        char task[10];
        memset(task, '\0', 10);
        sprintf(task, "%d", numSleep); // sleep time - 1, 2, 4, 8, 16, 32
	...
	// Some temp parameters
		...
		//cout << "num_tasks_this_package = " << num_tasks_this_package << endl;
			...
			alltasks.append(task_desc);	    alltasks.append("\'"); 	// Task Description
		...
		//cout << "str = " << str << endl;
                int32_t ret = clientRet.svrtosvr(str, str.length(), toserver); //cout << "sent = " << total_submitted1 << " ret = " << ret << endl;
		//sleep(1);
	...
	timespec diff = timediff(start_tasks, end_tasks); // Measure the total time to insert all tasks into Wait queue
...
int MATRIXClient::initializeTasks(int num_tasks_req, int numSleep, int computeSlots, int max_tasks_per_package, ZHTClient &clientRet, int DAG_choice){
